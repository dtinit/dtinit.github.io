...
<br>DTP - when run a server in a data exporter's platform - makes it easier for people to switch services, or try new and innovative products, by improving the ease and speed of data portability. **Without DTP**, transferring a copy of data from one service to another can be a time consuming process, requiring a user to download a copy of their data to a local device and re-upload it to a new service. This can be particularly challenging for mobile-only users, those with limited bandwidth, and in markets where people pay-as-they-go for data usage. Direct service-to-service portability removes the need for local device storage and transfer–**enabling users to move their data directly between services**. 

As an Open Source project, DTP is a server that can be adopted by data exporters and data importers, and run in their environment, connected to front-end interfaces and backend APIs, to support data portability well.  The Open Source project makes this easier for companies as well by providing standard data formats and mechanisms for managing long-running portability "jobs".  Multiple data importing/exporting platforms have contributed adaptors, such that a platform newly adopting DTP can quickly arrive at multiple interoperating solutions in deployment.
